...
import re
excel_regex = re.compile(r'exports\.downloadSessionExcel =\s*async\s*\(req,\s*res\)\s*=>\s*\{.*?\s*res\.end\(\);\s*\}\s*catch\s*\(error\)\s*\{\s*res\.status\(500\)\.json\(\{\s*success:\s*false,\s*message:\s*error\.message\s*\}\);\s*\}\s*\};', re.DOTALL)

excel_and_endpoints = """exports.downloadSessionExcel = async (req, res) => {
  try {
...
new_code = excel_regex.sub(excel_and_endpoints, new_code)

with open('backend/src/controllers/attendanceController.js', 'w') as f:
    f.write(new_code)

print("Done python build")
