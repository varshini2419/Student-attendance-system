...
// Only replace inside recognizeFace
const recognizeIdx = c.indexOf('exports.recognizeFace');
const existingRecordIdx = c.indexOf("if (\\n      existingRecord &&\\n      existingRecord.status === 'Present'\\n    ) {", recognizeIdx);
const endIdxStr = "message: 'Attendance marked successfully'\\n    });";
const endIdx = c.indexOf(endIdxStr, existingRecordIdx);

if (recognizeIdx !== -1 && existingRecordIdx !== -1 && endIdx !== -1) {
  const before = c.substring(0, existingRecordIdx);
  const after = c.substring(endIdx + endIdxStr.length);
  c = before + recognizeReplacement.trim() + '\\n    });' + after;
...
`;
  ...
  c = c.replace(/exports\.downloadSessionExcel =\s+async \(req, res\) => \{[\s\S]*?res\.end\(\);\s+\} catch \(error\) \{\s+res\.status\(500\)\.json\(\{\s+success: false,\s+message: error\.message\s+\}\);\s+\}\s+\};/, newExcelFunc);
  
  fs.writeFileSync('backend/src/controllers/attendanceController.js', c);
  console.log('Successfully completed full rebuild!');
} else {
