...
let code = fs.readFileSync('backend/src/controllers/attendanceController.js', 'utf8');

// 1. Splice recognizeFace
const recognizeReplacement = `
    const studentIdStr = bestMatch._id.toString();

    // Phase 2/5: ActivityState Check with Configurable Cooldown
    const ActivityState = require('../models/ActivityState');
    let state = await ActivityState.findOne({ student: studentIdStr, session: session._id });
...
const recognizeIdx = code.indexOf('exports.recognizeFace = async');
const existingRecordIdx = code.indexOf("if (\\n      existingRecord &&\\n      existingRecord.status === 'Present'\\n    ) {", recognizeIdx);
const endIdxStr = "message: 'Attendance marked successfully'\\n    });";
const endIdx = code.indexOf(endIdxStr, existingRecordIdx);

if (recognizeIdx !== -1 && existingRecordIdx !== -1 && endIdx !== -1) {
  const before = code.substring(0, existingRecordIdx);
  const after = code.substring(endIdx + endIdxStr.length);
  code = before + recognizeReplacement.trim() + '\\n    });\\n' + after;
} else {
  console.log('Error splicing recognizeFace');
  process.exit(1);
}

// 2. Replace downloadSessionExcel
const newExcelFunc = `
...
const excelRegex = /exports\.downloadSessionExcel =\s+async \(req, res\) => \{[\s\S]*?res\.end\(\);\s+\} catch \(error\) \{\s+res\.status\(500\)\.json\(\{\s+success: false,\s+message: error\.message\s+\}\);\s+\}\s+\};/;
if (excelRegex.test(code)) {
  code = code.replace(excelRegex, newExcelFunc);
} else {
  console.log('Error splicing downloadSessionExcel');
  process.exit(1);
}

// 3. Append missing endpoints safely via literal strings without template literal string interpolation issues
const endpointsStr = \`
...
fs.writeFileSync('backend/src/controllers/attendanceController.js', code);
console.log('Successfully completed building final attendanceController.js');
