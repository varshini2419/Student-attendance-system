...
// The replacement for recognizeFace (lines 442 to 533)
const recognizeStr = `    let student = bestMatch._id;
...
// 1. Manually splice recognizeFace block
...
if (recStart !== -1 && recEnd !== -1) {
  let newLines = [...lines.slice(0, recStart), recognizeStr, ...lines.slice(recEnd)];
  code = newLines.join('\\n');
  console.log('Spliced recognizeFace.');
}

// 2. Replace excel using standard regex since it's the last export in phase 0
const excelRegex = /exports\\.downloadSessionExcel =[\\s\\S]+/;
const excelAndNewEndpoints = `exports.downloadSessionExcel = async (req, res) => {
  try {
...
code = code.replace(excelRegex, excelAndNewEndpoints);
console.log('Appended Excel and new endpoints.');
